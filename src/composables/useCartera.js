...
export function useCartera() {
  ...
  async function loadFiles(input) {
    ...
    if (!selected.length) { error.value = 'Selecciona uno o varios archivos PDF válidos.'; return }
    loading.value = true
    error.value = ''
    invoices.value = []
    ...
    try {
      ...
      if (!invoices.value.length) error.value = 'No se encontraron documentos tipo P con el formato esperado.'
    } catch (cause) {
      console.error(cause)
  ...
  function onDrop(event) { dragActive.value = false; loadFiles(event.dataTransfer.files) }
  ...
  const filtered = computed(() => {
    ...
    return enriched.value.filter((row) => {
      const text = !needle || `${row.client} ${row.nit} ${row.document}`.toLocaleLowerCase('es').includes(needle)
      return text && (statusFilter.value === 'Todos' || row.status.key === statusFilter.value) && (termFilter.value === 'Todos' || row.term.key === termFilter.value)
    })
  })
  const summary = computed(() => {
    ...
    return { documents: rows.length, suppliers: new Set(rows.map((row) => row.nit)).size, total: sum(rows), overdue: sum(overdue), overdueCount: overdue.length, next30: sum(next30), next30Count: next30.length }
  })
  ...
  function toggleColumn(key) {
    ...
    selectedColumns.value = EXCEL_COLUMNS.filter((column) => next.has(column.key)).map((column) => column.key)
  }
  function selectAllColumns() { selectedColumns.value = EXCEL_COLUMNS.map((column) => column.key) }

  async function exportExcel() {
    if (!filtered.value.length || !selectedColumns.value.length) return
    loading.value = true; error.value = ''
    try { await exportCarteraExcel({ rows: filtered.value, files: files.value, cutoff: cutoff.value, selectedColumnKeys: selectedColumns.value }) }
    catch (cause) { console.error(cause); error.value = 'No fue posible generar el archivo Excel. Intenta nuevamente.' }
    finally { loading.value = false }
  }

  return { files, invoices, loading, error, query, statusFilter, termFilter, cutoff, dragActive, money, filtered, summary, excelColumns: EXCEL_COLUMNS, selectedColumns, loadFiles, onDrop, exportExcel, toggleColumn, selectAllColumns, formatDate }
}
